...
#ifndef BALLOON_WINDKESSEL_H
#define BALLOON_WINDKESSEL_H
...
/* BeginDocumentation
Name: balloon_windkessel - Simple conductance based leaky integrate-and-fire neuron model.

Description:
balloon_windkessel is an implementation of a spiking neuron using IAF dynamics with
...
SeeAlso: iaf_cond_exp, balloon_windkessel_mc
*/
...
namespace nest
{
   ...
   *       It would be better to use receptor_types, cf balloon_windkessel_mc.
   */
  class balloon_windkessel : public Archiving_Node
  {
  ...
  public:
    ...
    balloon_windkessel();
    balloon_windkessel(const balloon_windkessel&);
    ~balloon_windkessel();
  ...
  private:
    ...
    friend class RecordablesMap<balloon_windkessel>;
    friend class UniversalDataLogger<balloon_windkessel>;
...
*/
  ...
  private:
     ...
     */
    struct Buffers_ {
      Buffers_(balloon_windkessel&); //!<Sets buffer pointers to 0
      Buffers_(const Buffers_&, balloon_windkessel&); //!<Sets buffer pointers to 0

      //! Logger for all analog data
      UniversalDataLogger<balloon_windkessel> logger_;
    ...
    static RecordablesMap<balloon_windkessel> recordablesMap_;
  };
  
// Boilerplate inline function definitions ----------------------------------

inline port
balloon_windkessel::send_test_event( Node& target, rport receptor_type, synindex, bool )
{
  SpikeEvent e;
  e.set_sender( *this );
...
inline port
balloon_windkessel::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
...
inline port
balloon_windkessel::handles_test_event( CurrentEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
    throw UnknownReceptorType( receptor_type, get_name() );
  return 0;
...
inline port
balloon_windkessel::handles_test_event( DataLoggingRequest& dlr, rport receptor_type )
{
  if ( receptor_type != 0 )
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
balloon_windkessel::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d );
...
inline void
balloon_windkessel::set_status( const DictionaryDatum& d )
{
