...
class STDPHomeostaticConnection : public Connection< targetidentifierT >
{
...
template < typename targetidentifierT >
inline void
STDPHomeostaticConnection< targetidentifierT >::send( Event& e,
  thread t,
  double t_lastspike,
  const CommonSynapseProperties& )
{
  ...
  // incremented by Archiving_Node::register_stdp_homeostatic_connection(). See bug #218 for
  // details.
  target->get_history(
  ...
  while ( start != finish )
  {
    ...
    weight_ = facilitate_( weight_, Kplus_ * std::exp( minus_dt / tau_plus_ ) ) + wout_;
  }
...
STDPHomeostaticConnection< targetidentifierT >::get_status( DictionaryDatum& d ) const
{
  ...
  def< double >( d, names::wout, wout_ );
  def< long >( d, names::size_of, sizeof( *this ) );
}

template < typename targetidentifierT >
void
STDPHomeostaticConnection< targetidentifierT >::set_status( const DictionaryDatum& d,
  ConnectorModel& cm )
{
  ...
  updateValue< double >( d, names::wout, wout_ );
